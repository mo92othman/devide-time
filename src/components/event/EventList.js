...
function EventList() {
  ...
  return (
    <div className="mt-4">
      ...
      <ul className="list-none pl-4 w-full md:w-3/4 lg:w-full mx-auto">
        {events.map((event) => (
          <li
            key={event.id}
            className={`mb-2 cursor-pointer text-cyan-800 hover:bg-indigo-200 transform duration-100 ease-in-out p-2 rounded-md flex justify-between items-center ${
          ...
          >
            ...
            <img
              src={deleteIcon}
              alt="Delete Icon"
              className="w-4 h-4 cursor-pointer transform hover:scale-150 mr-3"
              onClick={(e) => {
